...
// Local shape stub — original imported CorpusStyleModel from ./style-model
// (corpus calibration), which is intentionally not ported. The corpus-aware
// branches below stay typecheckable but resolve to no-ops at runtime because
// loadStyleModel() always returns null.
interface CorpusStyleModel {
  sentenceLengthDistribution: { p90: number };
  transitionWordFrequency: Record<string, number>;
  contractionFrequency: { mean: number };
}

function loadStyleModel(): CorpusStyleModel | null {
  return null;
}
...
function injectHumanVoice(text: string, model: CorpusStyleModel): string {
  ...
  if (currentPer1000 < contractionMean && chance(0.5)) {
    ...
    for (const [expanded, contracted] of expansions) {
      ...
      if (expanded === undefined || contracted === undefined) continue;
  ...
  if (chance(0.10)) {
    ...
    const targetParagraph = paragraphs[pIdx] ?? '';
    const sentences = splitSentences(targetParagraph);
    if (sentences.length > 1) {
      ...
      if (targetSentence !== undefined) {
        sentences[sIdx] = conjunction + ' ' + targetSentence.charAt(0).toLowerCase() + targetSentence.slice(1);

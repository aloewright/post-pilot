...
function looksLikeProperNoun(
  ...
  if (position === 0) {
    return false; // First word of sentence is always capitalized
  }
  if (!/^[A-Z]/.test(word)) {
    return false;
  }
...
function reorderSentences(paragraph: string): string {
  ...
  if (sentences.length <= 2) {
    return paragraph; // Too short to reorder
  }

  const pronounPattern =
    /\b(he|she|it|they|this|that|these|those|his|her|its|their)\b/i;
  ...
  const swapCount = Math.max(
    1,
    Math.floor(middle.length * (0.2 + Math.random() * 0.1))
  );
  const result = [...middle];
  ...
  for (let s = 0; s < swapCount; s++) {
    ...
    if (
      pronounPattern.test(sentenceI.split(" ")[0] ?? "") ||
      pronounPattern.test(sentenceJ.split(" ")[0] ?? "")
    ) {
  ...
  return [sentences[0] ?? "", ...result, sentences.at(-1) ?? ""].join(" ");
}
    ...
    if (wc > 30 && chance(0.3)) {
      ...
      for (const pattern of breakPatterns) {
        ...
        if (
          ...
          match.index > 10 &&
          match.index < text.length - 10
        ) {
          breakPoint = match.index;
          // Start new sentence with the conjunction
          _replacement =
            ". " +
            match[0].replace(/,\s+/, "").charAt(0).toUpperCase() +
      ...
      if (breakPoint === -1) {
        ...
        const middleCommas = commas.filter(
          (c) =>
            c.index !== undefined &&
            c.index > text.length * 0.3 &&
            c.index < text.length * 0.7
        );
...
function randomizeParagraphs(text: string): string {
  ...
  for (let i = 0; i < paragraphs.length; i++) {
    ...
    if (sentences.length >= 4 && chance(0.2)) {
    ...
    if (chance(0.05)) {
      ...
      if (sentences.length >= 3) {
        const emphasizeIdx =
          1 + Math.floor(Math.random() * (sentences.length - 2));
...
function aggressiveSynonymSwap(text: string): string {
  const replacements: [RegExp, string[]][] = [
    ...
    [
      /\bconsequently\b/gi,
      ["so", "which means", "as a result", "because of that"],
    ],
    ...
    [
      /\bessentially\b/gi,
      ["basically", "at its core", "when you get down to it"],
    ],
    ...
    [
      /\bunprecedented\b/gi,
      ["never seen before", "completely new", "totally unusual"],
    ],
    ...
    [
      ...
      ["matters a lot", "is really important", "makes a big difference"],
    ],
    [
      /\bplays? an important role\b/gi,
      ["matters", "is important", "makes a difference"],
    ],
...
function disruptFlow(text: string): string {
  ...
  return paragraphs
    .map((p) => {
      ...
      if (chance(0.2) && result[0] !== undefined) {
        const conjunctions = [
          "And ",
          "But ",
          "So ",

...
export function successResponse(c: Context, data: unknown, status: number = 200) {
  return c.json({ success: true, data }, status as 200);
}

export function errorResponse(c: Context, error: string, status: number = 500, extra?: Record<string, unknown>) {
